...
def ray_search(dispatcher_constructor: Callable, # constructor for the dispatcher (e.g. INETDispatcher)
    ...
    def ray_trial(config, label, dispatcher_constructor, project_path, output_path, submit_constructor,
        ...
        storage_kwargs = None
        log_constructor = None
        log_kwargs = None

        if adv_path:
            storage_constructor = SQLiteStorage
            storage_kwargs = {'directory': adv_path, 'label': label}
            log_constructor = runtk.constructors.BatchtkLogger
            log_kwargs = {'file_out': f"{adv_path}/{label}.log"}
        ...
        return trial(
            ...
            log_constructor=log_constructor, log_kwargs=log_kwargs,
            report=report, cleanup=cleanup, check_storage=check_storage)
