...
class BaseFS(FS_Protocol):
    ...
    def atomic_write(self, file, data, mode='w'):
        fstmp = "{}.fs.tmp".format(file)
        with self.open(file, mode) as fptr:
            fptr.write(data)
            fptr.flush() # supported on python file objects and paramiko
            #https://docs.paramiko.org/en/stable/api/sftp.html#paramiko.sftp_file.SFTPFile
