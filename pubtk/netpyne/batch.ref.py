...
class Batch:
    def __init__(self, cfg, params, label):
        self.params = params
    ...
    def create_run(self):
        if
    def run(self):
        b = Batch(params=self.params, cfgFile=self.cfgFile, netParamsFile=self.netParamsFile, initCfg=self.initCfg)
        runBatch(b, self.batchLabel, setup='mpi_bulletin')
def sge_inet_run(params, runCfg):
    ray.init(
...
def sge_inet_run(config, dir, label):

    tid = tune.get_trial_id()
    tid = int(tid.split('-')[-1])
    dispatcher = INET_Dispatcher(cwd=dir, env=config, submit=)
def sge_run(config):
